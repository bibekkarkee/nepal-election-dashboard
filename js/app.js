...
// Summary cards (seats per party)
function renderSeatSummary(data) {
    const container = document.getElementById("summaryCards"); container.innerHTML = "";
    const summary = data.reduce((acc,row) => {
        if (!row.PoliticalPartyName) return acc;
        if (!acc[row.PoliticalPartyName]) acc[row.PoliticalPartyName] = { seats: 0 };
        acc[row.PoliticalPartyName].seats += 1;
        return acc;
    },{});
...
// Hot Seats (top candidates)
function renderHotSeats(data) {
    const container = document.getElementById("hotSeatsContainer"); container.innerHTML = "";
    const hot = data.filter(c => c.R === 1).slice(0, 10);
    hot.forEach(c => {
        const div = document.createElement("div"); div.className = "card";
        div.innerHTML = `<strong>${c.DistrictName}</strong> - ${c.CandidateName} (${c.PoliticalPartyName})<br>Votes: ${c.TotalVoteReceived}`;
        container.appendChild(div);
    });
}

// Render provinces map
...
function highlightProvinceMap(data) {
    ...
    provinceCards.forEach(card => {
        ...
        const topCandidate = data.filter(d => d.StateName === prov && d.R === 1)[0];
        card.style.background = topCandidate ? "#065f46" : "#1f2937";
    });
}
